...
CALL DELTA_T(KIDIA,KFDIA,KLON,KTDIA-1,ZP,PT(:,KTDIA),ZDU,&
 ...
 & ZDEOTA1(:,KTDIA-1),YDPHY,YDPHY3,ZTRLI,ZIRHOV)
DO JLON=KIDIA,KFDIA
...
DO JLEV=KTDIA,KLEV
  ...
  CALL DELTA_T(KIDIA,KFDIA,KLON,JLEV,PAPRSF(:,JLEV),PT(:,JLEV),ZDU,&
   & ZT_UW,ZT_US,ZT_US_IRHOV,ZT_UC,ZT_U,ZT_PU,ZT_TU,&
   & ZDEOTA1(:,JLEV),YDPHY,YDPHY3,ZTRLI,ZIRHOV)
  DO JLON=KIDIA,KFDIA
...
DO JLEV=KLEV,KTDIA,-1
  ...
  CALL DELTA_T(KIDIA,KFDIA,KLON,JLEV,PAPRSF(:,JLEV),PT(:,JLEV),ZDU,&
   & ZT_UW,ZT_US,ZT_US_IRHOV,ZT_UC,ZT_U,ZT_PU,ZT_TU,&
   & ZUEOTA1(:,JLEV),YDPHY,YDPHY3,ZTRLI,ZIRHOV)
  DO JLON=KIDIA,KFDIA
...
CALL DELTA_T(KIDIA,KFDIA,KLON,KTDIA-1,ZP,PT(:,KTDIA),ZDU,&
 & ZT_UW,ZT_US,ZT_US_IRHOV,ZT_UC,ZT_U,ZT_PU,ZT_TU,&
 & ZUEOTA1(:,KTDIA-1),YDPHY,YDPHY3,ZTRLI,ZIRHOV)
DO JLON=KIDIA,KFDIA
  PUEOTI(JLON,KTDIA-1)=&
  ...
  DO JLEV1=KTDIA-1,KLEV-1  ! initial half level
    ...
    DO JLEV2=JLEV1+1,ILEV  ! final half level
      ...
      CALL DELTA_T(KIDIA,KFDIA,KLON,JLEV2,PAPRSF(:,JLEV2),PT(:,JLEV2),ZDU,&
       & ZT_UW,ZT_US,ZT_US_IRHOV,ZT_UC,ZT_U,ZT_PU,ZT_TU,&
       & ZDEL1,YDPHY,YDPHY3,ZTRLI,ZIRHOV)
...
SUBROUTINE DELTA_C(KIDIA,KFDIA,KLON,KL,PRES,PT,PDU,PUW,PUS,PUS_IRHOV,PUC,&
 & P_U,P_PU,P_TU,PDELTA,YDPHY,YDPHY3,PTRLI,PIRHOV,PC_FW,PC_FS,PC_FC)
...
REAL(KIND=JPRB),    INTENT(IN)    :: PC_FW    (KLON,0:KLEV,3)
REAL(KIND=JPRB),    INTENT(IN)    :: PC_FS    (KLON,0:KLEV,3)
REAL(KIND=JPRB),    INTENT(IN)    :: PC_FC    (KLON,0:KLEV)
...
DO JG=1,3
  DO JLON=KIDIA,KFDIA
    ...
    PUW(JLON,JG)=PUW(JLON,JG)+PC_FW(JLON,KL,JG)*PDU(JLON,JG)
    PUS(JLON,JG)=PUS(JLON,JG)+PC_FS(JLON,KL,JG)*PDU(JLON,JG)*ZMD
    PUS_IRHOV(JLON,JG)=PUS_IRHOV(JLON,JG)+PIRHOV(JLON,KL)*&
     &                        PC_FS(JLON,KL,JG)*PDU(JLON,JG)*ZMD
...
DO JLON=KIDIA,KFDIA
  PUC(JLON)=PUC(JLON)+PC_FC(JLON,KL)*PDU(JLON,4)*ZMD
